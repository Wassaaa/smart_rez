...
function SmartRez:AddCraftSalvageWhitelistItemForSelection(professionKey, selection, itemID, skipRefresh, contextKey)
...
function SmartRez:AddCraftSalvageWhitelistItem(professionKey, itemID, skipRefresh, contextKey, selectionOverride)
  return self:AddCraftSalvageWhitelistItemForSelection(
    professionKey,
    selectionOverride or self:GetCraftSalvageSelection(professionKey),
    itemID,
    skipRefresh,
    contextKey
  )
...
function SmartRez:AddCraftSalvageReagentWhitelistItemForSelection(professionKey, selection, dataSlotIndex, itemID,
                                                                  skipRefresh, contextKey)
  if not professionKey or not dataSlotIndex or not itemID or not self.craftSalvageProfessions[professionKey] then
    return
  end

  local reagentSlots = selection and selection.reagentSlots or {}
...
function SmartRez:AddCraftSalvageReagentWhitelistItem(professionKey, dataSlotIndex, itemID, skipRefresh, contextKey,
                                                      selectionOverride)
  return self:AddCraftSalvageReagentWhitelistItemForSelection(
    professionKey,
    selectionOverride or self:GetCraftSalvageSelection(professionKey),
    dataSlotIndex,
    itemID,
...
function SmartRez:GetCraftSalvageAllowedReagentItems(professionKey, dataSlotIndex, contextKey)
  ...
  for _, reagentSlot in ipairs(reagentSlots) do
    if reagentSlot.dataSlotIndex == dataSlotIndex then
      return buildAllowedItemSet(reagentSlot.allowedItemIDs,
        self:GetCraftSalvageReagentWhitelist(professionKey, dataSlotIndex, contextKey), true)
    end
  end
