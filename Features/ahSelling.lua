...
local function getCandidatePostPrice(lowestPrice, minPrice)
  local normalizedMin = normalizePostPrice(minPrice)
  local normalizedLowest = normalizePostPrice(lowestPrice)

  if not normalizedMin then
    return nil
  end

  if not normalizedLowest then
    return normalizedMin
  end

  if normalizedLowest < normalizedMin then
    return nil
  end

  return normalizedLowest
end
...
local function prepareActiveScanPost(candidatePrice, lowestIsPlayer, frontQuantity)
  ...
  local quantity, alreadyPosted = getPostQuantity(activeScan, lowestIsPlayer, frontQuantity)
  if quantity <= 0 then
    line(
  ...
  debugLine(
    ...
    "x" .. tostring(quantity),
    "ready at",
    SmartRez.UI.FormatMoney(candidatePrice),
    "click again"
  )
end
...
local function executePreparedPost()
  ...
  if post.isCommodity then
    ok, needsConfirmationOrError = pcall(
  ...
  if needsConfirmationOrError and post.isCommodity then
    if AuctionHouseFrame
        and AuctionHouseFrame.CommoditiesSellFrame
        and AuctionHouseFrame.CommoditiesSellFrame.CachePendingPost then
      pcall(
        AuctionHouseFrame.CommoditiesSellFrame.CachePendingPost,
  ...
  elseif needsConfirmationOrError then
    ...
    elseif (not post.isCommodity)
        and AuctionHouseFrame
        and AuctionHouseFrame.ItemSellFrame
        and AuctionHouseFrame.ItemSellFrame.CachePendingPost then
      pcall(
...
local function findCommodityPostCandidate(itemID, minPrice)
  ...
  local firstPrice = first and normalizePostPrice(first.unitPrice) or nil

  -- Simple safety behavior:
  -- If anything is currently below our configured selling min, do not post.
  -- This includes our own bait auctions.
  if firstPrice and firstPrice < normalizedMin then
    return nil, false, 0, first, 1, "belowMinBlocked"
  end

  -- No market data, or no visible auctions: post at min.
  if not firstPrice then
...
local function processCommodityScan(itemID)
  ...
  local first = resultCount > 0 and C_AuctionHouse.GetCommoditySearchResultInfo(itemID, 1) or nil
  local firstPrice = first and first.unitPrice or nil
  local firstIsPlayer = first and first.containsOwnerItem and first.owners and first.owners[1] == "player"

  if first then
    debugLine(
      "lowest",
      "unit",
      tostring(first.unitPrice),
      "qty",
      tostring(first.quantity),
      "owner",
      firstIsPlayer and "player" or tostring(first.owners and first.owners[1])
    )
  end
  ...
  if candidateReason == "belowMinBlocked" then
    debugLine(
      "post blocked",
      "lowest below min",
      "lowest",
      candidateResult and SmartRez.UI.FormatMoney(candidateResult.unitPrice) or "nil",
      "min",
      SmartRez.UI.FormatMoney(activeScan.minPrice)
    )

    line(
      activeScan.itemLink or ("item:" .. tostring(activeScan.itemID)),
      colorText(
        "waiting, lowest below min " ..
        SmartRez.UI.FormatMoney(candidateResult and candidateResult.unitPrice or 0) ..
        " < " ..
        SmartRez.UI.FormatMoney(activeScan.minPrice),
        "FFD866"
      )
  ...
  if candidateResult then
    ...
    debugLine("candidate", "using min", "unit", tostring(candidatePrice))
  end
  ...
  if candidatePrice then
    debugLine(
      "post candidate",
      "stack",
      tostring(activeScan.stackSize),
      "unit",
      SmartRez.UI.FormatMoney(candidatePrice)
    )
    prepareActiveScanPost(candidatePrice, candidateIsPlayer, candidateQuantity)
  else
...
local function processItemScan(itemKey)
  ...
  if first then
    debugLine(
      "lowest",
      "buyout",
      tostring(first.buyoutAmount),
      "bid",
      tostring(first.bidAmount),
      "qty",
      tostring(first.quantity),
      "owner",
      lowestIsPlayer and "player" or tostring(first.owners and first.owners[1]),
      "auctionID",
      tostring(first.auctionID)
    )
  end

  recordLatestScan(activeScan.itemID, lowestPrice, activeScan.minPrice)

  local candidatePrice = getCandidatePostPrice(lowestPrice, activeScan.minPrice)
  if candidatePrice then
    debugLine(
      "post candidate",
      "stack",
...
function SmartRez:ScanAHSellingNextItem(allowPreparedPost)
  ...
  debugLine(
    "scan start",
    candidate.itemLink or ("item:" .. tostring(candidate.itemID)),
    "key",
    formatItemKey(candidate.itemKey),
    "commodity",
    tostring(candidate.isCommodity),
    "min",
    SmartRez.UI.FormatMoney(candidate.minPrice),
    "stack",
    tostring(candidate.stackSize)
  )
...
ahSellingFrame:SetScript("OnEvent", function(_, eventName, ...)
  ...
  elseif eventName == "UI_ERROR_MESSAGE" and pendingPost then
    ...
    line(
      pendingPost.itemLink or ("item:" .. tostring(pendingPost.itemID)),
      "post error",
      tostring(message or errorType or "unknown")
    )
    pendingPost = nil
