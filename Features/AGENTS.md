...
- AH Selling stack size is the preferred normal post size, but the last spendable partial stack below that size should still be postable so a configured item can drain to zero after keep-in-bags reserves are respected.
...
- Gold Printer salvage steps may store per-target and per-reagent item minimums under the step selection. These minimums are "keep at least this many" thresholds; salvage planning should only spend counts above the configured minimum so `untilNoTargets` stops before draining protected targets or reagents.
