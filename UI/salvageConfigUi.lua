...
local function renderCraftSalvageWhitelistSections(parent, profession, selection, contextKey, titlePrefix, config)
  ...
  UI.RenderIconMultiPicker(parent, {
    ...
    summaryText = UI.Colorize("79C0FF",
      "Whitelist entries: " ..
      tostring(UI.GetItemSetCount(SmartRez:GetCraftSalvageWhitelist(profession.key, contextKey)))),
    getSummaryText = function()
      return UI.Colorize("79C0FF",
        "Whitelist entries: " ..
        tostring(UI.GetItemSetCount(SmartRez:GetCraftSalvageWhitelist(profession.key, contextKey))))
    end,
  ...
  for _, reagentSlot in ipairs(selection.reagentSlots or {}) do
    UI.RenderIconMultiPicker(parent, {
      title = (titlePrefix or "") ..
          tostring(reagentSlot.label or ("Reagent Slot " .. tostring(reagentSlot.dataSlotIndex))),
      ...
      getSummaryText = function()
        return UI.Colorize("79C0FF", string.format(
          "Need: %d  |  Whitelist entries: %d",
          reagentSlot.quantityRequired or 0,
          UI.GetItemSetCount(SmartRez:GetCraftSalvageReagentWhitelist(profession.key, reagentSlot.dataSlotIndex,
            contextKey))
        ))
      end,
      ...
      addItemFunc = function(itemID)
        SmartRez:AddCraftSalvageReagentWhitelistItem(
          profession.key,
...
function UI.RenderCraftSalvageTab(parent, profession)
  ...
  if profession.key == "enchanting" then
    UI.RenderDisenchantWhitelist(parent, {
      ...
      controlHintText =
      "Click icons to choose what the manual Disenchant button is allowed to target. Leaving it empty means no manual disenchant targets are available.",
    })
  end
end
