...
local function renderGoldPrinterDisenchantWhitelist(parent, routineKey, stepIndex)
  UI.RenderDisenchantWhitelist(parent, {
    ...
    controlHintText =
    "Click icons to choose what this Gold Printer disenchant step may target. Leaving it empty means this step will have no work.",
  })
end

local function renderGoldPrinterCraftSalvageWhitelistSections(parent, routineKey, stepIndex, step)
  local selection = step and step.selection or nil
  ...
  if not profession or not selection then
    UI.AddLabel(parent, "Step " .. tostring(stepIndex) .. " has no configured salvage profession or selection yet.",
      "FFB86C")
    return
  end

  -- Important:
  -- The generic craft salvage whitelist UI validates additions through
  -- SmartRez:GetCraftSalvageSelection(professionKey).
...
local function renderRecipeCraftStepControls(stepGroup, stepIndex, step)
  UI.AddLabel(stepGroup,
    UI.Colorize("A5D6FF", "Recipe: ") .. tostring(step.recipeConfig and step.recipeConfig.label or "Unset"))

  if step.recipeConfig then
    UI.AddLabel(stepGroup,
      UI.Colorize("7EE787", "Output: ") ..
      select(1, UI.GetDisplayFromLinkOrID(step.recipeConfig.outputItemLink, step.recipeConfig.outputItemID)))
  end
  ...
  if step.recipeConfig then
    ...
    UI.AddLabel(stepGroup,
      "Turn this off only for recipes you have verified can craft directly by recipe ID without the profession window/backend being opened first.",
      "7D8590")
  end

  if step.recipeConfig and (step.recipeConfig.unsupportedRequiredSlot or false) then
    UI.AddLabel(stepGroup, "This recipe still has a required non-item reagent slot that Smart Rez cannot drive yet.",
      "FFB86C")
  end
end
...
local function renderCraftSalvageStepControls(stepGroup, stepIndex, step)
  ...
  if step.selection then
    ...
    UI.AddLabel(stepGroup,
      "Turn this off only for salvage recipes you have verified can craft directly without the profession window/backend being opened first.",
      "7D8590")
  end
end
...
local function renderGoldPrinterStep(parent, routineKey, stepTypeList, stepIndex, step)
  ...
  elseif step.type == "craftSalvage" then
    renderGoldPrinterCraftSalvageWhitelistSections(parent, routineKey, stepIndex, step)
  end
end
...
local function renderGoldPrinterGroup(parent)
  ...
  UI.AddLabel(goldPrinterGroup,
    "Build a routine out of recipe-craft, disenchant, and salvage steps. This first draft still reuses your existing recipe selections and buttons, but Gold Printer now has its own per-step whitelists.",
    "A5D6FF")
